...
import { setSelectedCompany, setSelectedOrganization, selectUser } from '@/lib/store/slices/userSlice';
...
const ChatSidebarClient = () => {
  ...
  const selectedCompany = useAppSelector((state) => state.user.selectedCompany) as Company & {
    assistants?: any[];
    chatConversations?: any[];
  };
  const selectedOrganization = useAppSelector(selectSelectedOrganization);
  const user = useAppSelector(selectUser);
  const userOrganizations = useAppSelector((state) => state.user.user?.organizations || []);
  ...
  const handleNewChat = (e: React.MouseEvent, assistantId?: string) => {
    ...
    const selectedAssistant = assistantId
      ...
      : availableAssistants.find((assist: any) => assist.name === 'report_agent') || availableAssistants[0];
  ...
  return (
    ...
    >
                    ...
                    {groupChats.map((chat, index) => (
                      ...
                      >
                        <div className="flex items-center space-x-2 truncate flex-1">
                          {!isSidebarOpen && <UserRoundCheck className="h-4 w-4 text-gray-500" />}
                          {isSidebarOpen && (
      ...
      {isOrgDropdownOpen && isSidebarOpen && (
        <>
          {/* Background overlay */}
          <div className="fixed inset-0 bg-black/30 z-40" onClick={() => toggleComponentState(orgDropdownId, false)} />
                    ...
                    return (
                      <div
                        ...
                        className={`group mx-1 my-0.5 flex items-center gap-3 rounded-md px-2.5 py-2 transition-colors cursor-pointer ${
                          isSelected ? 'bg-text-selected/10 text-text-selected' : 'text-gray-700 hover:bg-gray-50'
                        }`}
                        onClick={() => {
