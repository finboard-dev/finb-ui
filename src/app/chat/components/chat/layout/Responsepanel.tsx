...
const getPythonCode = (response: ToolCallResponse): string | null => {
  if (
    ...
    (response.type === "code" || response.type === "python")
  ) {
...
const saveComponentToLocalStorageInternal = (
  ...
  try {
    ...
    switch (componentType) {
      ...
      case "table":
        ...
        if (
          ...
          tableHtml.trim().startsWith("<table")
        ) {
    ...
    const componentData = {
      ...
      metadata: {
        ...
        ...(typeof response.data === "object" &&
          response.data?.schema && { hasSchema: true }),
      },
    };
...
const ResponsePanel: React.FC<ResponsePanelProps> = ({
  ...
  const { toolCallResponses, activeToolCallId } = useAppSelector(
  ...
  const displayedResponses = activeMessageId
    ? filteredResponsesForMessage
    : toolCallResponses;
  ...
  useEffect(() => {
    const handleToolCallSelected = (event: Event) => {
      ...
      const selectedResponse = toolCallResponses.find(
        (r) => r.tool_call_id === toolCallId
      );
      ...
      if (selectedResponse) {
        ...
        const hasInvalidDataFormatError =
          ...
          (typeof selectedResponse.data === "object" &&
            ...
            (selectedResponse.data.error === "Invalid data format provided" ||
              selectedResponse.data.localError ===
        ...
        if (
          selectedResponse.type === "graph" &&
          !canRenderGraph(selectedResponse.data)
        ) {
          return;
        }
        ...
        if (
          selectedResponse.type === "table" &&
          !canRenderTable(selectedResponse.data)
        ) {
          return;
        }
    ...
    return () =>
      window.removeEventListener("toolCallSelected", handleToolCallSelected);
  ...
  useEffect(() => {
    if (displayedResponses.length > 0) {
      ...
      if (!activeToolCallId || !currentToolCallStillValid) {
        const newActiveResponse =
          displayedResponses[displayedResponses.length - 1];
  ...
  const canRenderTable = (data: any): boolean => {
    try {
      ...
      if (data && typeof data === "object") {
        if (
          data.error === "Invalid data format provided" ||
    ...
    if (currentActiveResponse && responsePanelWidth > 0) {
      ...
      const hasInvalidDataFormatError =
        (typeof currentActiveResponse.data === "string" &&
          currentActiveResponse.data.includes(
            "Invalid data format provided"
          )) ||
        (typeof currentActiveResponse.data === "object" &&
          currentActiveResponse.data &&
          (currentActiveResponse.data.error ===
            "Invalid data format provided" ||
            currentActiveResponse.data.localError ===
              "Invalid data format provided"));
      ...
      if (
        currentActiveResponse.type === "graph" &&
        !canRenderGraph(currentActiveResponse.data)
      ) {
        dispatch(setResponsePanelWidth(0));
        return;
      }
  ...
  const handleSaveCurrentResponse = () => {
    if (currentActiveResponse) {
      ...
      if (successCount > 0) {
        toast.success(
          `Component saved to ${successCount} selected ${
            successCount === 1 ? "company" : "companies"
          }!`
        );
              ...
              <>
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center bg-[#F5F7FB] rounded-lg p-1 gap-1">
                    {getAvailableViews(currentActiveResponse).map(
                      (viewType) => (
                        <Button
                          key={viewType}
                          variant="ghost"
                          ...
                          aria-label={`${getViewLabel(
                            viewType,
                            currentActiveResponse.type
                          )} View`}
                        >
              ...
              <div className="w-full h-[450px] flex flex-col items-center justify-center text-center text-gray-500">
                ...
                {displayedResponses.length > 0 ? (
                  <p className="text-xs mt-1 text-gray-400">
                    Select an item from history to view output.
                  </p>
                ) : (
                  <p className="text-xs mt-1 text-gray-400">
