...
function CallbackHandler() {
  ...
  const [status, setStatus] = useState<
    "loading" | "success" | "add-company-success" | "error"
  >("loading");
  ...
  const selectedCompany = useAppSelector(
    (state) => state?.user?.selectedCompany || null
  );
  ...
  useEffect(() => {
    ...
    if (params.state && params.code) {
      ...
      setStatus("error");
      setMessage(
        "Invalid authentication request. Missing required parameters."
      );
  ...
  const handleLogin = async (params: { code: string }) => {
    ...
    try {
      ...
      if (response && response.token && response.user) {
        ...
        document.cookie = `has_selected_company=${!!response.user
          .selectedCompany}; path=/; SameSite=Lax; Secure`;
  ...
  const handleAddCompany = async (params: {
    realmId: string;
    code: string;
  }) => {
    setStatus("loading");
    ...
    try {
      ...
      if (response && response.success) {
        ...
        const userData = {
          ...
          selectedOrganization:
            response.selectedOrganization ||
            currentUserState.selectedOrganization,
          selectedCompany:
            response.selectedCompany ||
            currentUserState.selectedCompany ||
            response, // Prioritize newly added company as selected
        };
        ...
        if (userData.selectedCompany) {
          document.cookie =
            "has_selected_company=true; path=/; SameSite=Lax; Secure";
        }
        setTimeout(() => {
          // Simulate finalization
          setCurrentStepIndex(3);
      ...
      setStatus("error");
      setMessage(
        error.message || "Could not connect company. Please try again."
      );
      setDebug((prev) => ({
        ...prev,
        error: error.message,
      ...
      case "loading":
        return (
          ...
          >
            ...
            <motion.div
              variants={itemVariants}
              className="w-full max-w-md mx-auto space-y-3 px-4"
            >
            ...
            <motion.div
              variants={itemVariants}
              className="w-full max-w-md mx-auto space-y-1 px-4"
            >
      ...
      case "add-company-success":
        return (
          ...
          >
            ...
            <motion.div
              variants={itemVariants}
              className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-4 max-w-lg mx-auto"
            >
              <div
              ...
              >
                <User
                  className={`w-6 h-6 mx-auto mb-1 ${
                    currentUser?.id ? "text-green-600" : "text-gray-400"
                  }`}
                />
                <p className="text-xs font-medium">User Profile</p>
                <p
                  className={`text-xs ${
                    currentUser?.id ? "text-green-600" : "text-gray-500"
                  }`}
                >
              ...
              >
                ...
                <p
                  className={`text-xs ${
                    selectedCompany ? "text-green-600" : "text-gray-500"
                  }`}
                >
      ...
      case "error":
        return (
          ...
          >
            ...
            <motion.p
              variants={itemVariants}
              className="text-red-600 bg-red-50 p-3 rounded-md max-w-md mx-auto"
            >
              {message}
            </motion.p>
            <motion.div variants={itemVariants}>
