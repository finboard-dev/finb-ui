...
import { CreateAccounts, CreateAccountsRef } from '../components/stepper/CreateAccounts';
...
export default function ConsolidationPage() {
  ...
  const realmId = companies.find((company: any) => company.id === selectedCompanyId)?.realmId;
  ...
  if (isSyncing) {
    return (
      <div className="flex items-center justify-center h-screen w-screen bg-transparent">
        <LoadingAnimation message={isSyncing ? 'Initializing...' : 'Loading company data...'} />
      </div>
    );
  }
  ...
  const renderCurrentStep = () => {
    switch (currentStep) {
      ...
      case 1:
        return (
          <LinkAccounts ref={linkAccountsRef} onNext={() => setCurrentStep(2)} selectedCompanyId={selectedCompanyId} />
        );
      case 2:
        return <ReviewFinalize selectedCompanyId={selectedCompanyId} />;
      case 3:
        return <AdjustEliminations onNext={() => setCurrentStep(4)} selectedCompanyId={selectedCompanyId} />;
      default:
        return null;
    }
  };
  ...
  return (
    <>
      <div className="flex bg-white h-screen  overflow-hidden">
        ...
        <ConsolidationMain>
          <ConsolidationHeader collpaseSidebar={handleSidebarToggle} isCollapsed={isSidebarCollapsed} />
          <Stepper
