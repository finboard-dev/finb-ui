...
import { LayoutGridIcon, Rows3Icon, Code2Icon, GripVerticalIcon, ComponentIcon } from "lucide-react";

import type { Block, DraggingBlock, DashboardItem } from "../../types";
...
function BlockListItem({ block, onDragStart }: BlockListItemProps) {
    ...
    const renderPreview = (b: Block) => {
        ...
        if (b.type === "Visualization") icon = <LayoutGridIcon className="w-10 h-10 text-slate-400" />;
        else if (b.type === "SQL") icon = <Rows3Icon className="w-10 h-10 text-slate-400" />;
        else if (b.type === "Python") icon = <Code2Icon className="w-10 h-10 text-slate-400" />;
        return (
            <div className="w-full h-32 bg-slate-100 group-hover:bg-slate-200 transition-colors rounded-t-md flex flex-col items-center justify-center text-slate-500 p-2 text-center border-b border-slate-200">
                {icon}
                <span className="text-xs mt-2 line-clamp-2">{b.title || "Untitled Component"}</span>
    ...
    const onDragStartHandler = useCallback(
        ...
        [block.id, block.type, block.title, onDragStart]
    );
        ...
        >
            ...
            <CardHeader className="p-3">
                <div className="flex justify-between items-center">
                    <CardTitle className="text-sm font-semibold text-gray-800 truncate max-w-[calc(100%-50px)]" title={block.title || "Untitled"}>
                        {block.title || "Untitled"}
                    </CardTitle>
                    <span className="text-xs bg-slate-100 px-1.5 py-0.5 rounded-sm text-slate-600 border border-slate-200 flex-shrink-0">
                        {block.type}
                    </span>
                </div>
            </CardHeader>
...
interface DashboardControlsProps {
    ...
    onDragStart: (draggingBlock: DraggingBlock) => void;
    // dashboardItems and setDashboardItems removed as they are not directly used for core functionality here
}
...
export default function DashboardControls({
        ...
        try {
            ...
            if (storedBlocksString) {
                ...
                setBlocks((prevGlobalBlocks) => {
                    ...
                    const newBlocksToAdd = parsedStoredBlocks.filter(sb => !globalBlockIds.has(sb.id));
                    return newBlocksToAdd.length > 0 ? [...prevGlobalBlocks, ...newBlocksToAdd] : prevGlobalBlocks;
                });
            }
        } catch (error) {
    ...
    const handleDragOver = useCallback((e: React.DragEvent) => {
        e.preventDefault();
        if (e.dataTransfer.types.includes("application/dashboard-item-id")) setDragOverTarget(true);
    }, []);
    ...
    const handleDropOnControls = useCallback((e: React.DragEvent) => {
        ...
        const dashboardItemId = e.dataTransfer.getData("application/dashboard-item-id");
        if (dashboardItemId) {
            console.log("Item dropped on controls for deletion (TBD):", dashboardItemId);
            toast.info(`Item (ID: ${dashboardItemId.substring(0,8)}) drag-delete TBD.`);
        }
    }, []);
    ...
    const filterAndSearchBlocks = (blocksToFilter: Block[]): Block[] => {
        ...
        if (activeViewFilter) {
            if (activeViewFilter === "visual") filtered = filtered.filter(b => b.type === "Visualization" || b.type === "DashboardHeader");
            else if (activeViewFilter === "code") filtered = filtered.filter(b => b.type === "Python");
            else if (activeViewFilter === "table") filtered = filtered.filter(b => b.type === "SQL");
        }
        return searchQuery ? filtered.filter(b => b.title.toLowerCase().includes(searchQuery.toLowerCase())) : filtered;
    };
    ...
    const globalDisplayComponents = filterAndSearchBlocks(
        blocks.filter(b => ["Visualization", "SQL", "Python", "DashboardHeader"].includes(b.type) && !savedBlockIds.has(b.id))
    );
    ...
    if (!isOpen) {
        return (
            <div className="fixed right-0 top-1/2 -translate-y-1/2 z-30">
                <Button
                    onClick={() => setIsOpen(true)} variant="outline"
                ...
                >
                    ...
                    <span className="[writing-mode:vertical-rl] transform rotate-180 text-xs font-medium tracking-wider uppercase">Components</span>
                </Button>
            </div>
        );
    }
    ...
    return (
        <aside
            ...
            onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDropOnControls}
        >
            <div className="bg-slate-50 border-b border-slate-200 py-4 px-4 flex justify-between items-center h-[65px] flex-shrink-0">
                ...
                <Button onClick={() => setIsOpen(false)} variant="ghost" size="icon" className="text-slate-500 hover:text-slate-700 hover:bg-slate-200 p-1.5 rounded-md" aria-label="Hide components panel">
                    <ChevronLeftIcon className="w-5 h-5" />
                </Button>
            </div>
            <Tabs value={activeMainTab} onValueChange={setActiveMainTab} className="flex flex-col flex-grow min-h-0">
                <TabsList className="grid grid-cols-2 p-1.5 border-b border-slate-200 bg-slate-50 rounded-none flex-shrink-0 w-full">
                    {["My Components", "Global Components"].map(tabName => (
                        <TabsTrigger key={tabName} value={tabName.toLowerCase().replace(/\s+/g, '-')}
                ...
                <div className="p-3 border-b border-slate-200 bg-slate-50 flex-shrink-0">
                    <div className="relative mb-2.5">
                        ...
                        <Input type="text" placeholder="Find by name" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                    ...
                    <div className="grid grid-cols-3 gap-2 items-center justify-start w-full">
                        {[{ label: "Visual", value: "visual", icon: LayoutGridIcon }, { label: "Code", value: "code", icon: Code2Icon }, { label: "Table", value: "table", icon: Rows3Icon }]
                            .map((filter) => (
                                <Button key={filter.value} variant={activeViewFilter === filter.value ? "secondary" : "outline"} size="sm"
                                        onClick={() => setActiveViewFilter(prev => prev === filter.value ? "" : filter.value)}
                                        className={cn("px-2 py-1 h-8 text-xs rounded-md flex items-center justify-center gap-1.5 w-full transition-colors",
                                            activeViewFilter === filter.value ? "bg-blue-600 text-white hover:bg-blue-700 border-blue-600" : "text-slate-600 bg-white border-slate-300 hover:bg-slate-100 hover:border-slate-400")}
                                ><filter.icon className="w-3.5 h-3.5" />{filter.label}</Button>
                            ))}
                    </div>
                </div>
                <ScrollArea className="flex-1 min-h-0">
                    <TabsContent value="my-components" className="p-3 focus-visible:ring-0 focus-visible:ring-offset-0">
                        {myDisplayComponents.length > 0 ? myDisplayComponents.map((block) => (
                            <BlockListItem key={block.id} block={block} onDragStart={onDragStart} />
                        )) : <p className="text-sm text-slate-500 text-center py-10">No 'My Components' found.</p>}
                    </TabsContent>
                    <TabsContent value="global-components" className="p-3 focus-visible:ring-0 focus-visible:ring-offset-0">
                        {globalDisplayComponents.length > 0 ? globalDisplayComponents.map((block) => (
                            <BlockListItem key={block.id} block={block} onDragStart={onDragStart} />
                        )) : <p className="text-sm text-slate-500 text-center py-10">No 'Global Components' found.</p>}
                    </TabsContent>
                </ScrollArea>
            </Tabs>
