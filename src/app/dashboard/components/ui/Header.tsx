...
import { Share2Icon, PlusIcon, MoreHorizontalIcon, EyeIcon, EditIcon, SaveIcon } from "lucide-react";
import {
    DropdownMenu,
    DropdownMenuContent,
    DropdownMenuItem,
        ...
        <div className="bg-white border-b border-gray-200 px-4 md:px-6 py-3 flex items-center justify-between sticky top-0 z-20 h-[65px] flex-shrink-0">
            <div className="flex items-center gap-4 flex-1 min-w-0">
                {currentDashboardName && !isEditing ? (
                    <h1 className="text-lg font-semibold text-slate-800 truncate" title={currentDashboardName}>
                        {currentDashboardName}
                    </h1>
                ) : (
                    <Tabs defaultValue={activeTab} onValueChange={setActiveTab} className="w-auto">
                        <TabsList className="bg-gray-100 p-1 h-auto rounded-lg">
                            ...
                            <TabsTrigger value="add-new-tab" className="px-2 sm:px-2.5 py-1.5 text-gray-500 hover:text-blue-600 data-[state=active]:bg-white data-[state=active]:shadow-sm data-[state=active]:text-blue-600 rounded-md">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
                ...
                >
                    {isEditing ? <EyeIcon className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> : <EditIcon className="w-3.5 h-3.5 sm:w-4 sm:h-4" />}
                ...
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <Button variant="outline" size="icon" className="w-9 h-9 border-gray-300">
                    ...
                    <DropdownMenuContent align="end" className="bg-white shadow-xl border-slate-200 z-50">
                        <DropdownMenuItem className="text-sm cursor-pointer text-slate-700 hover:!bg-slate-100 focus:!bg-slate-100">Export Dashboard</DropdownMenuItem>
                        <DropdownMenuItem className="text-sm cursor-pointer text-slate-700 hover:!bg-slate-100 focus:!bg-slate-100">Dashboard Settings</DropdownMenuItem>
                    </DropdownMenuContent>
                </DropdownMenu>
            </div>
        </div>
