...
export const CompanyModal: React.FC<OrganizationModalProps> = ({
  onCompanyChange,
  ...
  const [disconnectingCompanyId, setDisconnectingCompanyId] = useState<
    string | null
  >(null);
  ...
  const modalState = useAppSelector(
    (state) => state.ui.components[componentId]
  );
  ...
  const multiEntityCompanies = companies.filter(
    (company) => company.isMultiEntity
  );

  // Get active companies for sub-entities selection
  const activeCompanies = normalCompanies.filter(
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      ...
      if (
        connectPopupCompanyId &&
        connectPopupRefs.current[connectPopupCompanyId]
      ) {
  ...
  const handleSelectAllSubEntities = () => {
    if (selectAllSubEntities) {
      ...
      setSelectedSubEntities(
        allNonMultiEntityCompanies.map((company) => company.id)
      );
          ...
          {showQuickBooksPopup && (
            ...
            >
              ...
              >
                ...
                />
                <Image
                  src={connectToQuickBooksHoverMed}
                  alt="Connect to QuickBooks Hover"
                  width={110}
                  height={28}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {connectPopupCompanyId === company.id && (
                          <div
                            ...
                            className="absolute right-2 mt-8 z-20 bg-white rounded-md shadow-lg p-2 flex flex-col items-center min-w-[170px] border border-gray-200"
                          >
                            <button
                              onClick={() => {
                                setConnectPopupCompanyId(null);
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              ...
              <div className="flex items-center">
                ...
                <label
                  htmlFor="select-all"
                  className="text-xs font-medium text-gray-700 cursor-pointer ml-1.5"
                >
                  Select All
                </label>
              </div>
      ...
      <div className="border-t border-gray-200 bg-white p-4 flex-shrink-0">
        <div className="flex justify-end gap-2">
          ...
          <button
            ...
            disabled={
              createMultiEntityMutation.isPending ||
              !createForm.company_name.trim() ||
              selectedSubEntities.length === 0
            }
